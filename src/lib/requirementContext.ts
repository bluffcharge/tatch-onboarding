...
export const REQUIREMENT_CONTEXT: Record<string, RequirementContext> = {
  ...
  "/operator/team?seats=3": {
    ...
    links: [
      { label: "Auto-upgrade state ↗", href: "/operator/team?seats=3&invites=4" },
      { label: "Armen's invite screen ↗", href: ARMEN_PROTOTYPE },
    ],
  },
  "/operator/team?seats=3&invites=4": {
    oneLine: "Step 6 — invites exceed seats → the plan auto-adjusts, priced in the open.",
    flowRefs: [{ flow: OP_FLOW, step: "Step 6: seat auto-upgrade (US4, redesigned)" }],
    storyRefs: ["US4 · Invite team"],
    reqs: [
      "Design call 2026-06-09: the old warn → Confirm upgrade gate is gone; Add another just works.",
      "Badge grows to the billed count with a '+N · $/mo' chip; meter never reads over.",
      "Hairline note above the CTA: 'plan cost will increase to $X/mo from $Y/mo with Z extra seats.'",
      "CTA carries the running total ('Send invites & finish · $X/mo').",
    ],
