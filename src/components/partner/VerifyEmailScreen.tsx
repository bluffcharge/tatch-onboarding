...
/* Step 1, part two — verify the email address before it becomes the login.
   Still "Step 1 of 6" in the rail: proving the address is part of creating
...
export function VerifyEmailScreen() {
  ...
  const email = sp.get("email") || "jordan@northwindroofing.com";
  ...
  const verify = () => {
    if (!ready) return;
    router.push(`/partner/account/about?email=${encodeURIComponent(email)}`);
  };

  return (
    <PartnerShell step={1} backHref="/partner/account">
